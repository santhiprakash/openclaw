    ...
    OpenClawChatCommandOutbox
{
    ...
    public func loadSessions() async -> [OpenClawChatSessionEntry] {
        ...
        do {
            return try await databases.cacheQueue.write { db in
    ...
    public func loadTranscript(sessionKey: String) async -> [OpenClawChatMessage] {
        await loadTranscript(sessionKey: sessionKey, agentID: nil)
    }
    ...
    public func storeCanonicalTranscript(
        ...
        observeCanonicalMessageIdempotencyKeys(canonicalMessageIdempotencyKeys)
    ...
    public func mergeCanonicalTranscriptMessage(
        ...
        canonicalMessageIdempotencyKey: String
    ) async {
        observeCanonicalMessageIdempotencyKeys([canonicalMessageIdempotencyKey])
        guard !isRetired else { return }
        let normalizedAgentID = Self.normalizedAgentID(agentID)
        let gatewayID = self.gatewayID
        do {
    ...
    public nonisolated func observeCanonicalMessageIdempotencyKeys(_ keys: Set<String>) {
        canonicalMessageProofHub.observe(keys)
    }

    public func loadSessionRoutingIdentity() async -> OpenClawChatSessionRoutingIdentity? {
        guard !isRetired else { return nil }
        return databases.loadSessionRoutingIdentity(gatewayID: gatewayID)
    }
...
extension OpenClawChatSQLiteTranscriptCache {
    ...
    private nonisolated static func replaceTranscript(
        ...
        let stale = try Row.fetchAll(
            ...
            LIMIT -1 OFFSET \(maxCachedTranscripts)
            """,
            arguments: [gatewayID]
        )
        for row in stale {
...
public extension OpenClawChatSQLiteTranscriptCache {
    ...
    func enqueueCommand(_ command: OpenClawChatOutboxCommand) async -> Bool {
        ...
        let gatewayID = self.gatewayID
        do {
            return try await databases.stateQueue.write { db in
    ...
    func recoverInterruptedSends() async -> Bool {
        ...
        if hasRecoveredInterruptedSends {
            return true
        }
        let gatewayID = self.gatewayID
        do {
            ...
            hasRecoveredInterruptedSends = true
            return true
        } catch {
            return false
        }
    ...
    func claimNextCommand() async -> OpenClawChatOutboxCommand? {
        ...
        do {
            ...
            for scope in result.1 {
                outboxChangeHub.yield(.invalidated(gatewayID: gatewayID, scope: scope))
            }
            return result.0
        } catch {
    ...
    func markCommandQueued(
        ...
        retryCount: Int,
        lastError: String?
    ) async -> OpenClawChatOutboxUpdateResult {
    ...
    func markCommandRetriedIfPresent(
        ...
        guard !isRetired else { return .unavailable }
        ...
        do {
            return try await databases.stateQueue.write { db in
                ...
                try db.execute(
                    ...
                    arguments: [
                        updateID,
                        wasPossiblyAccepted ? 1 : expectedAttemptVersion + 1,
                        branchState.epoch,
                        Date().timeIntervalSince1970,
                        normalizedAgentID,
    ...
    func cancelCommand(id: String) async -> OpenClawChatOutboxUpdateResult {
        ...
        let gatewayID = self.gatewayID
        let proofHub = canonicalMessageProofHub
        do {
            ...
            if isProven {
                outboxChangeHub.yield(.confirmed(gatewayID: gatewayID, id: id))
                return .confirmed
            }
            outboxChangeHub.yield(.canceled(gatewayID: gatewayID, id: id))
            return .updated
        } catch {
            return .unavailable
        }
    ...
    ) async -> OpenClawChatOutboxBranchState? {
        guard !isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await databases.stateQueue.write { db in
    ...
    func updateLastActiveLeafEntryID(
        ...
        guard !isRetired, !leaf.isEmpty else { return false }
        let gatewayID = self.gatewayID
        do {
            return try await databases.stateQueue.write { db in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
    ...
    func reconcileBranchScope(
        ...
        lastError: String
    ) async -> [OpenClawChatOutboxCommand]? {
        let leaf = activeLeafEntryID?.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !isRetired, activeLeafEntryID == nil || leaf?.isEmpty == false else { return nil }
        let gatewayID = self.gatewayID
        do {
    ...
    func confirmBranchChange(
        ...
        let leaf = activeLeafEntryID.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !isRetired, !leaf.isEmpty else { return nil }
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> ([OpenClawChatOutboxCommand], Bool) in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
                ...
                if state.lastActiveLeafEntryID == leaf {
                    try Self.writeBranchState(
                        db, gatewayID: gatewayID, scope: scope,
                        epoch: state.epoch, lastActiveLeafEntryID: leaf
                    )
                    invalidated = state.switchPendingSince != nil
                } else {
                    try Self.installConfirmedBranchChange(
                        db, gatewayID: gatewayID, scope: scope,
                        previousEpoch: state.epoch, activeLeafEntryID: leaf, lastError: lastError
                    )
                    invalidated = true
                }
                return try (Self.readCommands(db, gatewayID: gatewayID), invalidated)
...
extension OpenClawChatSQLiteTranscriptCache {
    private func transitionClaimedCommand(
        ...
        guard !isRetired else {
            hasRecoveredInterruptedSends = false
            return .unavailable
        }
        let gatewayID = self.gatewayID
        do {
    ...
    private nonisolated static func applyOutboxStaleness(
        ...
        try db.execute(
            ...
            """,
            arguments: [
                outboxUnconfirmedError,
                outboxExpiredError,
                gatewayID,
                Date().timeIntervalSince1970 - outboxCommandMaxAge,
            ]
        )
    }
    ...
    private nonisolated static func readCommands(
        _ db: Database,
        gatewayID: String
    ) throws -> [OpenClawChatOutboxCommand] {
        let rows = try Row.fetchAll(
            db,
            sql: """
            SELECT c.*, s.branch_epoch AS scope_branch_epoch
    ...
    private nonisolated static func optionalAgentID(_ agentID: String) -> String? {
        let normalized = normalizedAgentID(agentID)
    ...
    private nonisolated static func readBranchState(
        ...
        gatewayID: String,
        scope: OpenClawChatOutboxScope
    ) throws -> OpenClawChatOutboxBranchState {
        guard let row = try Row.fetchOne(
            db,
            sql: """
    ...
    private nonisolated static func expireBranchSwitchLeases(
        _ db: Database,
        gatewayID: String,
        scope: OpenClawChatOutboxScope? = nil
    ) throws -> [OpenClawChatOutboxScope] {
    ...
    private nonisolated static func writeBranchState(
        ...
        try db.execute(
            ...
            arguments: [
                ...
                gatewayID,
                scope.sessionKey,
                normalizedAgentID(scope.agentID),
                expectedRevision,
                expectedRevision,
            ]
        )
    ...
    private nonisolated static func installConfirmedBranchChange(
        ...
        try Self.writeBranchState(
            db, gatewayID: gatewayID, scope: scope,
            epoch: nextEpoch, lastActiveLeafEntryID: activeLeafEntryID
        )
        try db.execute(
            sql: """
            UPDATE outbox_commands
            SET parked_was_accepted = CASE
                    WHEN status IN ('sending', 'awaiting_confirmation') OR had_unacknowledged_send = 1 THEN 1 ELSE parked_was_accepted END,
...
extension OpenClawChatSQLiteTranscriptCache {
    ...
    static func boundedSessions(_ sessions: [OpenClawChatSessionEntry]) -> [OpenClawChatSessionEntry] {
        guard sessions.count > maxCachedSessions else { return sessions }
        return Array(
            sessions
    ...
    static func canEnqueueAttachmentBytes(commandBytes: Int, queuedBytes: Int) -> Bool {
        guard commandBytes >= 0,
              queuedBytes >= 0,
              commandBytes <= maxAttachmentBytesPerCommand
        else { return false }
        return queuedBytes <= maxQueuedAttachmentBytes - commandBytes
    }
