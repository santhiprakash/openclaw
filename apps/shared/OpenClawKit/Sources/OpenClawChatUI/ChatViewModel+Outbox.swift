...
extension OpenClawChatViewModel {
    ...
    static func branchListingIsUnsupported(_ error: Error) -> Bool {
        ...
        return error.domain == "OpenClawChatTransport" && error.code == 0 &&
            error.localizedDescription == "sessions.branches.list not supported by this transport"
    }
        ...
        connectionGeneration: UInt64) async -> Bool
    {
        ...
        guard let commands = await outbox.reconcileBranchScope(
            scope, previousState: previousState, activeLeafEntryID: activeLeaf,
            branchLeafEntryIDs: leaves, activeTranscriptEntryIDs: activeTranscriptEntryIDs,
            lastError: "Session branch changed; review and retry this message.")
        else { return false }
        self.reconciledOutboxBranchScopes.insert(scope)
            ...
            for (scope, scopedCommands) in grouped where !self.reconciledOutboxBranchScopes.contains(scope) {
                ...
                do {
                    ...
                    guard let reconciled = await outbox.reconcileBranchScope(
                        scope, previousState: state, activeLeafEntryID: activeLeaf,
                        branchLeafEntryIDs: leaves, activeTranscriptEntryIDs: activeTranscriptEntryIDs,
                        lastError: "Session branch changed; review and retry this message.")
                    else { continue }
                    self.reconciledOutboxBranchScopes.insert(scope)
    ...
    public func retryOutboxMessage(_ messageID: UUID) {
        ...
        Task { [weak self] in
            ...
            let result = await outbox.markCommandRetriedIfPresent(
                id: commandID,
                expectedAttemptVersion: failure.attemptVersion,
                expectedRetryCount: failure.retryCount,
                expectedLastError: failure.lastError,
                agentID: agentID,
